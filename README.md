...
- 运营报销记录是运营全局页面。有效运营可查看彼此的已报销及未报销记录，默认查看“我的记录”，也可筛选全部或指定运营，明细显示所属运营姓名。查阅同事时仅提供查看入口；新增、删除未报销记录及确认报销仅限本人，并由数据库校验。公开读取和确认 RPC 通过安全包装进入私有实现；即使 `private` schema 不向普通登录会话开放，运营页面仍可使用，而私有函数本身继续拒绝直接调用。周期按上海时区每月 25 日至次月 24 日计算。“确认报销”弹窗列出本人当前及历史待报销周期，优先当前周期，否则选择最近的待报销周期；确认前显示该周期完整笔数与金额。历史补报不设截止时间，保留费用日期与原周期，报销时间为实际操作时间。费用日期按上海业务日期显示，报销时间和更新时间把完整时间戳转换为上海时间并显示到分钟，无效值显示 `-`。关键词停止输入 300ms 后才查询，运营、周期、状态和分页立即查询，恢复筛选会取消尚未执行的关键词查询；保存、删除或确认报销后的刷新始终使用页面当前生效的筛选，较早请求不能覆盖新结果。列表搜索及状态筛选不改变确认范围。列表由服务端按每页 20 条分页，费用汇总随运营筛选变化，周期选项与汇总不受当前页或 200 条上限截断。已报销记录保留作为凭证。
...
- Storage 上传必须核对服务端返回的准确对象路径，附件登记必须逐条核对父记录、对象路径和数据库编号；Storage 删除即使收到空数组，也必须先确认目标存在且当前账号可读，再逐项确认对象已经不存在。附件登记失败时会清理本次上传的孤立对象；数据库策略只允许上传者在自己有权管理的订单目录内读取这些待清理对象。注册、验证码确认和修改密码必须返回权威用户编号；找回密码邮件只显示“已开始发送”的提示状态，不能使用绿色成功表示已经投递。
