...
- `dashboard-home/`：各角色首页组件化工作台；问候、当前时间、个人邀请码、公告和个人待办都作为可重复放置的首页组件呈现。小于 1280px 时按账号已保存的组件数组顺序自动排列，390px 单列，640–1279px 两列且问候、待办和公告跨两列；“管理首页”弹窗可以添加、移除、恢复默认和调整先后顺序，尺寸继续由当前屏幕自动决定，不读取或改写桌面坐标。1280px 起使用账号保存的五列坐标、拖拽、边缘缩放和按钮式位置/尺寸调整。布局策略、保存状态 hook、组件卡片、桌面侧栏、窄屏管理弹窗、调整弹窗和内容渲染拆在同层模块中；时钟刷新、邀请码复制、待办查询、保存反馈、弹窗和展示继续拆分，不放进首页 Client/Page。
