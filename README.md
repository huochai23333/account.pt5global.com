...
- 运营报销记录是运营全局页面。有效运营可查看彼此的已报销及未报销记录，默认查看“我的记录”，也可筛选全部或指定运营，明细显示所属运营姓名。查阅同事时仅提供查看入口；新增、删除未报销记录及确认报销仅限本人，并由数据库校验。周期按上海时区每月 25 日至次月 24 日计算。“确认报销”弹窗列出本人当前及历史待报销周期，优先当前周期，否则选择最近的待报销周期；确认前显示该周期完整笔数与金额。历史补报不设截止时间，保留费用日期与原周期，报销时间为实际操作时间。列表搜索及状态筛选不改变确认范围。列表由服务端按每页 20 条分页，费用汇总随运营筛选变化，周期选项与汇总不受当前页或 200 条上限截断。已报销记录保留作为凭证。
...
- 本地运营报销回归使用两名种子运营账号，运行 `operator-reimbursements.spec.ts` 与 `operator-reimbursements-sharing.spec.ts`；协作套件要求 `NEXT_PUBLIC_SUPABASE_URL=http://127.0.0.1:54321`，只写入带本次测试标识的费用并在结束后清理，覆盖历史补报、205 条分页、相互查阅、查询失败重试及 1440/390/320px 排版。
