...
- `npm run test:regression` 会依次执行 lint、typecheck、双语静态检查、工作台结构检查、操作成功契约检查、AI 流终态测试、build 和 Playwright 回归测试。`config/operation-contracts.json` 是写入与后台任务的机器可读清单；新增数据库写入、写入型 RPC 或有副作用的 HTTP 请求如果未登记，或没有校验实际回执，会直接阻止回归。完整浏览器验收由 Playwright 独占一个开发服务器并使用 `--workers=1`，避免测试夹具相互覆盖。
...
- 运营报销记录是运营全局页面。有效运营可查看彼此的已报销及未报销记录，默认查看“我的记录”，也可筛选全部或指定运营，明细显示所属运营姓名。查阅同事时仅提供查看入口；新增、删除未报销记录及确认报销仅限本人，并由数据库校验。周期按上海时区每月 25 日至次月 24 日计算。“确认报销”弹窗列出本人当前及历史待报销周期，优先当前周期，否则选择最近的待报销周期；确认前显示该周期完整笔数与金额。历史补报不设截止时间，保留费用日期与原周期，报销时间为实际操作时间。费用日期按上海业务日期显示，报销时间和更新时间把完整时间戳转换为上海时间并显示到分钟，无效值显示 `-`。关键词停止输入 300ms 后才查询，运营、周期、状态和分页立即查询，恢复筛选会取消尚未执行的关键词查询；保存、删除或确认报销后的刷新始终使用页面当前生效的筛选，较早请求不能覆盖新结果。列表搜索及状态筛选不改变确认范围。列表由服务端按每页 20 条分页，费用汇总随运营筛选变化，周期选项与汇总不受当前页或 200 条上限截断。已报销记录保留作为凭证。
...
- 登录、语言切换、路由跳转、刷新、上传、审核和保存动作都要有明确等待反馈，并避免重复点击。批发订单的创建、修改和结汇要把数据库写入与订单列表刷新视为同一个等待周期；刷新完成前按钮保持禁用、弹窗保持打开，同一个动作不能再次进入请求。
...
- 所有后台运行通过同一个 `operationId` 关联触发、Worker/Edge 尝试、数据库结果和页面状态。页面只有在终态为 `succeeded` 时显示成功；排队或运行中显示处理中，等待超过 90 秒显示“结果确认中”，部分失败和失败保留各自状态。管理员从 `/admin/system-health` 查看完成时间、成功/失败数量、重试次数和需处理提醒；导航角标只统计未恢复提醒。
