...
- 邮件工作台固定显示在管理员和业务员左侧工作栏，地址分别为 `/admin/mail` 与 `/salesman/mail`；其他角色无入口且接口拒绝。全公司只连接 `chinapt5@gmail.com`，业务员只绑定飞书并使用系统界面。业务员只能读取自己负责的正常会话，当前负责人可转交给其他启用业务员，也可把本人会话移入隔离区；管理员可查看全部、待指派、隔离区、收件规则、人员配置、邮箱健康和 AI 报告。
...
- 业务员别名和 Ref 前缀在首次使用时按账号名称自动生成；中文或无效名称回退到登录邮箱前缀，再回退到稳定的用户短码。重复值追加稳定短码，别名最多 40 位、Ref 前缀为 2–16 位大写字母数字。业务员可修改或恢复自己的建议值，管理员可修改所有业务员；账号名称后续变化不会覆盖已保存设置。发件显示名与签名都以带认证标签的密文保存，签名可以留空且仍能正常读取。只有已启用发件资料的业务员账号可以提交新邮件和回复；管理员在未具备业务员发件资料时只能整理、指派和生成可编辑草稿，页面会保持发送按钮不可用。
