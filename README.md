...
- 本地验收必须先恢复 Supabase 线索夹具，再用单个开发服务和单 worker Playwright 检查管理员、两名业务员、财务和客户的入口及业务边界，同时覆盖 375px 移动宽度和桌面宽度。双业务员竞争用例必须等待每一条认领请求的权威响应；已被认领的请求应立即显示明确提示，不能把尚在执行的请求当作竞争结束，也不能等待到网关超时后才显示通用失败。
...
- 账号没有任何已启用业务时统一进入 `/business-unavailable`。该页面位于工作台布局之外，不加载导航、业务查询或 AI 助手，并提供帮助邮箱和退出登录。
