...
- 业务员别名和 Ref 前缀在首次使用时按账号名称自动生成；中文或无效名称回退到登录邮箱前缀，再回退到稳定的用户短码。重复值追加稳定短码，别名最多 40 位、Ref 前缀为 2–16 位大写字母数字。业务员可修改或恢复自己的建议值，管理员可修改所有业务员；账号名称后续变化不会覆盖已保存设置。只有已启用发件资料的业务员账号可以提交新邮件和回复；管理员在未具备业务员发件资料时只能整理、指派和生成可编辑草稿，页面会保持发送按钮不可用。
