...
- 邮件工作台固定显示在管理员和业务员左侧工作栏，地址分别为 `/admin/mail` 与 `/salesman/mail`；其他角色无入口且接口拒绝。全公司只连接 `chinapt5@gmail.com`，业务员只绑定飞书并使用系统界面。业务员只能读取自己负责的会话，当前负责人可转交给其他启用业务员；管理员可查看全部、待指派、人员配置、邮箱健康和 AI 报告。只有已启用发件资料的业务员账号可以提交新邮件和回复；管理员在未具备业务员发件资料时只能整理、指派和生成可编辑草稿，页面会保持发送按钮不可用。
...
- `tests/e2e/mail.spec.ts` 直接使用本地 PT5 Supabase，并只模拟 Gmail、飞书、附件扫描和 DeepSeek 外部边界。用例覆盖左侧入口、其余六类角色拒绝、收件已读、AI 建议编辑、回复、新建、附件、转交、管理员报告、删除系统副本，以及 Gmail 返回编号但独立查询没有 `SENT` 时必须失败；并在 1440/390/320px 检查横向溢出。真实 Google OAuth、真实收发可见性和真实飞书到达需要在生产切换前单独验收。
