...
- 收件过滤只改变 PT5 工作台数据，不修改 Gmail 规则、标签或邮件原件。明确放行优先于明确隔离；其后才保护已有正常业务会话并识别带 `List-Unsubscribe`、`List-Id` 或 `Precedence: bulk/list/junk` 的群发邮件。自动回复标记本身不会触发隔离。隔离邮件继续加密保存，但不会指派、生成新 Ref、发送飞书提醒、计入待办/时效/人员分布或进入 AI 上下文；管理员恢复时重新路由并取得版本、Ref、通知数量和审计凭证，删除系统副本仍保留 Gmail 原件。
- 管理员可维护发件邮箱、发件域名和主题关键词的放行/隔离规则，并批量隔离、恢复或删除系统副本。规则内容加密保存，浏览器角色不能直接访问规则表。批量请求逐条携带预期版本；任一会话过期时整批不写入，规则创建仅部分完成时页面明确显示部分失败。
...
- `scripts/mail-intake.test.mjs` 覆盖自动别名、Ref、规则规范化和收件优先级；`tests/e2e/mail.spec.ts` 直接使用本地 PT5 Supabase，并只模拟 Gmail、飞书、附件扫描和 DeepSeek 外部边界。页面用例覆盖左侧入口、其他角色拒绝、个人与管理员发件设置、规则创建、业务员隔离、管理员批量隔离/恢复/删除、数据库版本与审计、收件已读、AI 建议编辑、回复、新建、附件、转交、管理员报告、HTTP 200 但隔离 0 行更新，以及 Gmail 返回编号但独立查询没有 `SENT` 时必须失败；并在 1440/390/320px 检查邮件、隔离区和规则页横向溢出。真实 Google OAuth、真实收发可见性和真实飞书到达需要在生产切换前单独验收。
