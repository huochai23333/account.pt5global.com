...
- 邮件工作台固定显示在管理员和业务员左侧工作栏，地址分别为 `/admin/mail` 与 `/salesman/mail`；其他角色无入口且接口拒绝。全公司只连接 `chinapt5@gmail.com`，业务员只绑定飞书并使用系统界面。业务员只能读取自己负责的会话，当前负责人可转交给其他启用业务员；管理员可查看全部、待指派、人员配置、邮箱健康和 AI 报告。
