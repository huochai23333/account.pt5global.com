...
export default async function CompanyTemplateGuidePage({ params }: {
  ...
  const template = (await listCompanyTemplates(await getServerSupabaseClient()))
    .find((item) => item.id === templateId && item.status === "active" && item.currentVersion.guide_sha256);
  if (!template) notFound();
  const t = await getTranslations("CompanyTemplates");
  return <CompanyTemplateViewer guide template={template} text={(key) => t(key)} workspace={workspace} />;
}
