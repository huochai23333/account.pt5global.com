  ...
  return (
    ...
    >
      <div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(19rem,0.8fr)_minmax(0,1.6fr)]">
        ...
        <MailThreadDetailPanel agents={state.enabledAgents} aiDraft={state.aiDraft} busy={state.busy} canDelete={props.isAdmin} composer={state.composer} detail={state.selected} onAssign={(id) => void state.assign(id)} onComposer={state.setComposer} onDelete={() => void state.deleteSelected()} onFiles={(files) => void state.uploadFiles(files)} onSend={() => void state.send()} onState={(value) => void state.updateState(value)} onSuggest={() => void state.generateReply()} />
      </div>
